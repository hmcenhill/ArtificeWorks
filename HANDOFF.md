...
**Last updated:** 2026-07-18 (story 3.4)
...
- **Story 3.4 done**: integration coverage closed. New `ProductApiTests` (create/duplicate-409/get/get-404) plus additions to `WorkOrderApiTests` (history-404, hold-already-held-409, advance-while-held-409, full-lifecycle walk asserting history at each step, concurrent-create smoke check). Extracted a shared `ApiTestExtensions.ReadProblemCodeAsync` for the ProblemDetails `code` contract. Every endpoint now has ≥1 success + ≥1 failure test; every `ProblemCodes` value is asserted. **68 tests green (45 unit + 23 integration).** Full reasoning in `docs/Plan/.../3.4.md`. **Epic 3 complete.**

## Next up

1. Epic 4 — the async pivot (message-driven lifecycle). First async epic; see `docs/Plan/`.
