...
- [x] With pacing on, one order takes tens of seconds to reach Completed and visibly rests in
      each status on the way
- [x] No handler, workflow service or event contract sleeps or changes shape
- [x] A paced order and an unpaced order reach **identical** end state — asserted, not assumed
- [x] The consumer keeps running other orders while one is paced; throughput does not collapse to
      one order at a time
- [x] A paced message keeps its routing key, correlation id and trace context across the delay
- [x] Pacing off is the shipped default and the existing suite passes untouched
