...
- [x] With generation on, orders appear at roughly the configured rate and the board is never empty
- [x] The generator never exceeds `MaxInFlight` orders in flight
- [x] Every work order carries a persisted origin, queryable and exposed on the DTO
- [x] Simulated and visitor activity are separable in logs, traces, metrics and `/system/stats`
- [x] Generation is off by default; the existing suite passes untouched
- [x] Nothing the generator does can stop the API, the worker or the simulation host
