...
- [x] The world can be reset on a schedule and on demand, and both do exactly the same thing
- [x] A reset never interrupts, alters or deletes an order in flight
- [x] Component stock returns to seed levels and repeated resets are idempotent
- [x] Old terminal, held and faulted orders are retired with no orphaned rows behind them
- [x] The catalog and the dead-letter table are never touched
- [x] A factory left running with generation on does not run out of stock or fill up with orders
