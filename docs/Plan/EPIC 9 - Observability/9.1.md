...
- [x] Add the OpenTelemetry packages and a shared `AddArtificeWorksTelemetry(...)` extension in
...
- [x] Instrumentation from the box: ASP.NET Core, HttpClient, EF Core / Npgsql. Turn on
...
- [x] **Trace context across AMQP.** Inject `traceparent`/`tracestate` into the message headers on
...
- [x] **Carry trace context through the outbox** — this is the story's real work:
...
- [x] **Correlation id becomes baggage** (the decision taken at grooming). The 4.3 correlation id
      ...
      set it
- [x] Domain attributes on the spans that have them: `artificeworks.work_order_id`,
...
- [x] The retry ladder must not fragment the trace. A message that climbs a rung and comes back is
...
- [x] `grafana/otel-lgtm` in `docker-compose.yml` (OTLP on 4317/4318, Grafana on 3000), with the
...
- [x] Tests: a publish→consume round trip over a real broker (Testcontainers, the existing
...
- [x] One work order driven from `POST` to Completed produces **one** trace spanning both services
- [x] Trace context survives the outbox — no trace ends at a commit
- [x] Spans carry the correlation id and the work order id as attributes
- [x] A retried delivery joins the original trace rather than starting a new one
- [x] Both services export to a single OTLP endpoint brought up by `docker compose up -d`
- [x] With no collector reachable, both hosts start and run normally
