...
public static class ChaosServiceCollectionExtensions
{
    ...
    /// <em>registry</em>, which its inspection service consults to fire a <c>FailInspection</c>
    /// fault; the service travels along harmlessly. The repository is <c>TryAdd</c>ed so this can be
    /// called alongside <c>AddWorldReset</c> (which needs the same registry for its disarm sweep)
